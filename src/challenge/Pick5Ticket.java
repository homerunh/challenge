/**
 *  Ryan Harrigan
 */
package challenge;

/**
 * 
 * This is the Pick5Ticket Class.  Its purpose is to :
 * 
 * Represent a Pick5 ticket.
 * 
 * @author Ryan Harrigan
 * @version Apr 21, 2016
 *
 */
public class Pick5Ticket extends LotteryTicket {

  /**
   * Create a new Pick5Ticket object.
   *
   * @param lotteryNumbers
   */
  public Pick5Ticket(int[] lotteryNumbers) {
    super(LotteryTicketType.PICK5, lotteryNumbers);
  }
  
  /**
   * Describe the lottery ticket as a String.
   * 
   * This is different than the super class because we want to represent
   * this ticket as a 'single number'.
   * 
   * @return - String representation of this ticket.
   */
  @Override
  public String toString() {
    int length = getLotteryNumbersCount();
    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < length; i++) {
      sb.append(getLotteryNumbers()[i]);
    }
    
    return sb.toString();
  }
}
